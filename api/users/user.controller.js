...
exports.getUserList = function *() {
    ...
    let itemsPerPage = (parseInt(this.query.itemsPerPage) > 0) ? parseInt(this.query.itesPerPage) : 10;
    ...
    try {
        ...
        let userList = yield User.find({}).skip(startRow).limit(itemPerPage).sort(sortName).exec();
        this.status = 200;
...
exports.addUser = function *() {
    ...
    if (nickname === '') {
        ...
        error_msg = '昵称不合法'
    }else if(email.length <=4 || email.length > 15 || !EMAIL_REGEXP.test(email)  ){
        error_msg = '邮箱地址不合法'
    }
    ...
    try{
        ...
        yield Logs.create({
            ...
            content:'创建用户' + (user.email || user.nickname),
            type:'user'
        });
        this.status = 200;
...
exports.destroy = function *() {
    ...
    if(String(userId) === String(this.params.id)){
        this.status = 403;
        return this.body = {message:'不能删除自己已经登陆的用户'}
    }else{
        try{
...
exports.updateUser = function *() {
    ...
    try{
        ...
        user.nickname = this.rquest.body.nickname;
...
exports.getUserProvider = function *() {
    try{
        ...
        this.status = 200;
        this.body = {success:true,data:user.providerInfo};
    }catch(err){
        this.throw(err)
    }
}
